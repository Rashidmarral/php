...
<form method="post" action="/app/subcontract-payments/<?= $payment['id'] ?>" id="payment-form">
  <?= csrf_field() ?>

  <div class="card" style="max-width:600px;">
    <div class="form-row">
      <div class="form-group"><label><?= t('user.subcontract_payments.date') ?></label><input type="date" name="payment_date" value="<?= e($payment['payment_date']) ?>"></div>
      <div class="form-group"><label><?= t('user.subcontract_payments.retention_percent') ?></label><input type="number" step="0.01" min="0" max="100" name="retention_percent" id="retention-percent" value="<?= e((string)$payment['retention_percent']) ?>"></div>
    </div>
    ...
    <div class="form-group">
      ...
      <input type="number" step="0.01" name="cumulative_value" id="cumulative-value" value="<?= e((string)$payment['cumulative_value']) ?>" min="<?= e((string)$previousCumulative) ?>" max="<?= e((string)$contractValue) ?>" required>
    </div>

    <div class="form-group"><label><?= t('user.subcontract_payments.notes') ?></label><textarea name="notes" rows="2"><?= e($payment['notes'] ?? '') ?></textarea></div>
  </div>
  ...
  <div class="card" style="margin-top:20px;max-width:420px;">
    <div class="breakdown" style="font-size:13.5px;">
      ...
      <div><span><?= t('user.subcontract_payments.retention') ?> (<span id="retention-pct-label">0</span>%)</span><span id="calc-retention">0.00</span></div>
    </div>
