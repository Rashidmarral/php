    ...
    <?php else: ?>
      ...
      <div id="library-picker-list" style="overflow-y:auto;flex:1;display:flex;flex-direction:column;gap:6px;">
        <?php foreach ($materials as $m): ?>
          <button type="button" class="library-picker-item" data-desc="<?= e($m['name']) ?>" data-desc-ar="<?= e($m['name_ar'] ?? '') ?>" data-cost="<?= (float) $m['unit_cost'] ?>"
            data-search="<?= e(strtolower($m['name'] . ' ' . ($m['category'] ?? '') . ' ' . ($m['supplier_name'] ?? ''))) ?>"
            ...
            <span>
              <strong style="display:block;font-size:13.5px;"><?= e(local($m, 'name')) ?></strong>
              <span class="help-text" style="font-size:12px;"><?= e($m['category'] ?: '—') ?> · <?= e($m['unit']) ?><?= $m['supplier_name'] ? ' · ' . e($m['supplier_name']) : '' ?></span>
              <span class="help-text" style="font-size:11.5px;">M <?= money((float)($m['material_cost'] ?? 0)) ?> + L <?= money((float)($m['labor_cost'] ?? 0)) ?></span>
            </span>
            <span class="badge badge-blue" style="white-space:nowrap;"><?= money((float)$m['unit_cost']) ?>/<?= e($m['unit']) ?></span>
          </button>
        <?php endforeach; ?>
      </div>
    <?php endif; ?>
