...
<div class="card" style="max-width:520px;margin-bottom:20px;">
  <h3 style="margin-bottom:4px;"><?= e($plan['name']) ?> plan</h3>
  <p class="help-text" style="margin-bottom:14px;"><?= ucfirst($cycle) ?> <?= t('user.billing.billing_suffix') ?></p>
  <div class="total-row"><?= money($amount) ?><?= $cycle === 'yearly' ? ' ' . t('user.billing.per_year') : ' ' . t('user.billing.per_month') ?></div>
</div>
  ...
  <?php if ($bankTransferEnabled): ?>
    <div class="card" style="max-width:520px;">
      ...
      <table class="data" style="margin:14px 0;">
        <tbody>
          <tr><td><?= t('user.billing.bank') ?></td><td><?= e($bank['name'] ?: '—') ?></td></tr>
          <tr><td><?= t('user.billing.account_name') ?></td><td><?= e($bank['accountName'] ?: '—') ?></td></tr>
          <tr><td><?= t('common.iban') ?></td><td><?= e($bank['iban'] ?: '—') ?></td></tr>
          <tr><td><?= t('user.billing.account_number') ?></td><td><?= e($bank['accountNumber'] ?: '—') ?></td></tr>
        </tbody>
      </table>
...
<div id="tab-card" class="tab-panel" style="display:none;">
  <div class="card" style="max-width:520px;">
    ...
    <div class="mysr-form"
      ...
      data-currency="SAR"
      data-description="<?= e($plan['name']) ?> plan (<?= e($cycle) ?>)"
      data-publishable-api-key="<?= e($moyasarPublishableKey) ?>"
      data-callback-url="<?= e('/app/billing/moyasar-callback?plan=' . urlencode($plan['slug']) . '&cycle=' . urlencode($cycle)) ?>"
