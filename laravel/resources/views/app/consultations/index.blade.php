  ...
  <div class="card" style="max-width:560px;margin-bottom:20px;">
    <div style="display:flex;justify-content:space-between;align-items:center;">
      <div>
        ...
        <?php if ($planName): ?><span class="help-text">(<?= e($planName) ?> plan)</span><?php endif; ?>
      </div>
    </div>
  ...
  <?php else: ?>
    <table class="data">
      ...
      <?php foreach ($consultations as $c): ?>
        <tr>
          <td class="help-text"><?= e($c['created_at']) ?></td>
          <td><?= $c['type'] === 'in_person' ? t('user.consultations.in_person_label') : t('user.consultations.chat_video_label') ?></td>
          <td><?= e($c['topic']) ?></td>
          <td><span class="badge badge-<?= $c['status']==='completed'?'green':($c['status']==='cancelled'?'red':($c['status']==='scheduled'?'blue':'yellow')) ?>"><?= e(ucfirst($c['status'])) ?></span></td>
          <td><?= e($c['assigned_engineer'] ?: '—') ?></td>
          <td class="help-text"><?= e($c['scheduled_at'] ?: ($c['preferred_date'] ?: '—')) ?></td>
          <td>
