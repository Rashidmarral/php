      ...
      <div>
        ...
        <div class="card" style="margin-bottom:18px;">
          ...
          <div class="qe-pick-grid" id="qe-tiers" style="grid-template-columns:repeat(3,1fr);">
            @foreach ($qualityTiers as $qt)
              @php($name = app()->getLocale() === 'ar' ? $qt['name_ar'] : $qt['name_en'])
        ...
        <div class="card" style="margin-bottom:18px;">
          ...
          <div class="qe-addon-grid" id="qe-addons">
            @foreach ($addons as $a)
              ...
              @php($manual = ($a['qty_mode'] ?? 'area') === 'manual')
                ...
                <div>
                  ...
                  @if($manual)
                    <div class="qe-addon-qty">
                      <span>{{ t('qe.qty') }} ({{ $a['unit_type'] }})</span>
        ...
        <div class="card" style="margin-bottom:18px;">
          <p class="help-text" style="margin-top:-4px;">📩 {{ t('qe.lead_capture_note') }}</p>
          <div class="form-row">
            <div class="form-group"><label>{{ t('qe.your_name') }}</label><input type="text" name="contact_name" required></div>
            <div class="form-group"><label>{{ t('auth.email') }}</label><input type="email" name="contact_email" required></div>
          </div>
          <div class="form-group"><label>{{ t('auth.phone') }}</label><input type="tel" name="contact_phone" placeholder="+966 5x xxx xxxx" required></div>
