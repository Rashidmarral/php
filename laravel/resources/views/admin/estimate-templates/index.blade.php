...
<table class="data">
  ...
  <?php foreach ($templates as $t): $fid = 'tpl-' . $t['id']; $itemWord = (int) $t['item_count'] === 1 ? t('admin.templates.item_singular') : t('admin.templates.item_plural'); ?>
    <form id="<?= $fid ?>" method="post" action="/admin/estimate-templates/<?= $t['id'] ?>"><?= csrf_field() ?></form>
    <tr>
      <td><input form="<?= $fid ?>" type="text" name="icon" value="<?= e($t['icon']) ?>" maxlength="10" style="width:48px;text-align:center;"></td>
      <td><input form="<?= $fid ?>" type="text" name="name_en" value="<?= e($t['name_en']) ?>" style="min-width:150px;"></td>
      <td><input form="<?= $fid ?>" type="text" name="name_ar" value="<?= e($t['name_ar']) ?>" dir="rtl" style="min-width:150px;"></td>
      <td><input form="<?= $fid ?>" type="text" name="building_type" value="<?= e($t['building_type'] ?? '') ?>" style="width:110px;"></td>
      <td><a href="/admin/estimate-templates/<?= $t['id'] ?>/items"><?= (int) $t['item_count'] ?> <?= $itemWord ?> →</a></td>
      <td><input form="<?= $fid ?>" type="number" name="sort_order" value="<?= e((string)$t['sort_order']) ?>" style="width:70px;"></td>
