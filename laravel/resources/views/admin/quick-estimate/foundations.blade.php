...
<table class="data">
  ...
  <?php foreach ($foundations as $f): $fid = 'foundation-' . $f['id']; ?>
    <form id="<?= $fid ?>" method="post" action="/admin/quick-estimate/foundations/<?= $f['id'] ?>"><?= csrf_field() ?></form>
    <tr>
      <td><input form="<?= $fid ?>" type="text" name="name_en" value="<?= e($f['name_en']) ?>" style="min-width:140px;"></td>
      <td><input form="<?= $fid ?>" type="text" name="name_ar" value="<?= e($f['name_ar']) ?>" dir="rtl" style="min-width:140px;"></td>
      <td><input form="<?= $fid ?>" type="number" step="0.01" name="price_per_sqm" value="<?= e((string)$f['price_per_sqm']) ?>" style="width:90px;"></td>
      <td><input form="<?= $fid ?>" type="number" name="sort_order" value="<?= e((string)$f['sort_order']) ?>" style="width:70px;"></td>
