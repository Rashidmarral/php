...
<form method="post" action="/admin/profile" class="card" style="max-width:520px;">
  ...
  <div class="form-group"><label><?= t('common.name') ?></label><input type="text" name="name" value="<?= e($admin['name']) ?>" required></div>
  <div class="form-group"><label><?= t('common.email') ?></label><input type="email" name="email" value="<?= e($admin['email']) ?>" required></div>
