...
<form method="post" action="/admin/settings" class="card" style="max-width:680px;">
  ...
  <div class="form-row">
    <div class="form-group"><label><?= t('admin.settings.site_name') ?></label><input type="text" name="site_name" value="<?= View::e($settings['site_name'] ?? 'BuildXact Saudi') ?>"></div>
    <div class="form-group"><label><?= t('admin.settings.currency') ?></label><input type="text" name="currency" value="<?= View::e($settings['currency'] ?? 'SAR') ?>"></div>
  </div>
  <div class="form-row">
    <div class="form-group"><label><?= t('admin.settings.support_email') ?></label><input type="email" name="support_email" value="<?= View::e($settings['support_email'] ?? '') ?>"></div>
    <div class="form-group"><label><?= t('admin.settings.support_phone') ?></label><input type="tel" name="support_phone" value="<?= View::e($settings['support_phone'] ?? '') ?>"></div>
  </div>
