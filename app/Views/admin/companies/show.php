...
<div class="kpi-grid">
  <div class="kpi"><div class="label"><?= t('common.status') ?></div><div class="value" style="font-size:16px;"><span class="badge badge-<?= $company['status']==='active'?'green':($company['status']==='suspended'?'red':'yellow') ?>"><?= View::e($company['status']) ?></span></div></div>
  ...
  <div class="kpi"><div class="label"><?= t('admin.company.trial_ends') ?></div><div class="value" style="font-size:16px;"><?= View::e($company['trial_ends_at'] ?: '—') ?></div></div>
</div>
...
<details class="card" style="margin-bottom:24px;max-width:720px;">
  ...
  <form method="post" action="/admin/companies/<?= $company['id'] ?>/profile" enctype="multipart/form-data" style="margin-top:16px;">
    <?= Csrf::field() ?>
    <div class="form-row">
      <div class="form-group"><label><?= t('admin.company.name_en') ?></label><input type="text" name="name" value="<?= View::e($company['name']) ?>"></div>
      <div class="form-group"><label><?= t('admin.company.name_ar') ?></label><input type="text" name="name_ar" dir="rtl" value="<?= View::e($company['name_ar'] ?? '') ?>" placeholder="اسم الشركة"></div>
    </div>
    <div class="form-row">
      <div class="form-group"><label><?= t('common.email') ?></label><input type="email" name="email" value="<?= View::e($company['email']) ?>"></div>
      <div class="form-group"><label><?= t('common.phone') ?></label><input type="tel" name="phone" value="<?= View::e($company['phone']) ?>"></div>
    </div>
    <div class="form-row">
      <div class="form-group"><label><?= t('admin.company.city') ?></label><input type="text" name="city" value="<?= View::e($company['city']) ?>"></div>
      <div class="form-group"><label><?= t('admin.company.address_freetext') ?></label><input type="text" name="address" value="<?= View::e($company['address'] ?? '') ?>"></div>
    </div>
    <div class="form-row">
      <div class="form-group"><label><?= t('admin.company.cr_number') ?></label><input type="text" name="cr_number" value="<?= View::e($company['cr_number']) ?>"></div>
      <div class="form-group"><label><?= t('admin.company.vat_number') ?></label><input type="text" name="vat_number" value="<?= View::e($company['vat_number']) ?>"></div>
    </div>
    ...
    <div class="form-row">
      ...
      <div class="form-group"><label><?= t('admin.company.classification_number') ?></label><input type="text" name="contractor_classification_number" value="<?= View::e($company['contractor_classification_number'] ?? '') ?>"></div>
    </div>

    <h3 style="font-size:13px;margin-top:16px;"><?= t('admin.company.zatca_address') ?></h3>
    <div class="form-row">
      <div class="form-group"><label><?= t('admin.company.building_number') ?></label><input type="text" name="building_number" maxlength="4" value="<?= View::e($company['building_number'] ?? '') ?>" placeholder="1234"></div>
      <div class="form-group"><label><?= t('admin.company.street_name') ?></label><input type="text" name="street_name" value="<?= View::e($company['street_name'] ?? '') ?>"></div>
    </div>
    <div class="form-row">
      <div class="form-group"><label><?= t('admin.company.district') ?></label><input type="text" name="district" value="<?= View::e($company['district'] ?? '') ?>"></div>
      <div class="form-group"><label><?= t('admin.company.postal_code') ?></label><input type="text" name="postal_code" maxlength="5" value="<?= View::e($company['postal_code'] ?? '') ?>" placeholder="12345"></div>
    </div>
    <div class="form-group" style="max-width:240px;"><label><?= t('admin.company.additional_number') ?></label><input type="text" name="additional_number" maxlength="4" value="<?= View::e($company['additional_number'] ?? '') ?>" placeholder="6789"></div>
    ...
    <div class="form-row">
      <div class="form-group">
        ...
        <?php if (!empty($company['cr_document_path'])): ?>
          <p class="help-text"><a href="<?= View::e($company['cr_document_path']) ?>" target="_blank" rel="noopener"><?= t('admin.company.view_uploaded_file') ?> →</a></p>
        <?php endif; ?>
      ...
      <div class="form-group">
        ...
        <?php if (!empty($company['vat_document_path'])): ?>
          <p class="help-text"><a href="<?= View::e($company['vat_document_path']) ?>" target="_blank" rel="noopener"><?= t('admin.company.view_uploaded_file') ?> →</a></p>
        <?php endif; ?>
...
<details class="card" style="margin-top:24px;max-width:520px;border-color:#e0958c;">
  ...
  <div style="margin-top:16px;">
    ...
    <form method="post" action="/admin/companies/<?= $company['id'] ?>/hard-delete" onsubmit="return confirm('This permanently deletes all of ' + <?= json_encode($company['name']) ?> + '\'s data. This cannot be undone. Continue?');">
      <?= Csrf::field() ?>
      <div class="form-group">
        <label><?= t('admin.company.confirm_name_prompt') ?> (<strong><?= View::e($company['name']) ?></strong>)</label>
