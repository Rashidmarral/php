...
<div class="page-head">
  <div>
    <h1>ZATCA e-invoicing — <?= View::e($company['name']) ?></h1>
  ...
  <a href="/admin/companies/<?= $company['id'] ?>" class="btn btn-secondary">← Back to company</a>
</div>

<div class="kpi-grid" style="margin-bottom:24px;">
  <div class="kpi">
    <div class="label">Onboarding status</div>
  ...
  <div class="kpi">
    <div class="label">Last invoice counter (ICV)</div>
    <div class="value"><?= (int) ($company['zatca_last_icv'] ?? 0) ?></div>
  </div>
</div>

<?php if (!empty($company['zatca_last_error'])): ?>
  <div class="alert alert-error" style="margin-bottom:24px;"><strong>Last error:</strong> <?= View::e($company['zatca_last_error']) ?></div>
<?php endif; ?>

<div class="card" style="margin-bottom:24px;max-width:520px;">
  <h3>1. About Phase 1 vs Phase 2</h3>
  <p class="help-text">
  ...
  <form method="post" action="/admin/companies/<?= $company['id'] ?>/zatca/environment" style="display:flex;gap:8px;align-items:end;">
    ...
    <div class="form-group" style="margin:0;flex:1;">
      ...
      <select name="zatca_environment">
        <option value="sandbox" <?= ($company['zatca_environment'] ?: 'sandbox') === 'sandbox' ? 'selected' : '' ?>>Sandbox (developer portal, testing)</option>
        <option value="production" <?= $company['zatca_environment'] === 'production' ? 'selected' : '' ?>>Production (live, real invoices)</option>
      </select>
    </div>
...
<div class="card" style="margin-bottom:24px;max-width:640px;">
  <h3>3. Generate CSR (certificate signing request)</h3>
  ...
  <form method="post" action="/admin/companies/<?= $company['id'] ?>/zatca/csr" onsubmit="return confirm('Generating a new CSR will replace any existing one. Continue?');">
    <?= Csrf::field() ?>
    <button type="submit" class="btn btn-primary" <?= empty($company['vat_number']) ? 'disabled' : '' ?>>Generate CSR</button>
  </form>
  <?php if (!empty($company['zatca_csr'])): ?>
    <div style="margin-top:14px;">
      <label>Current CSR (PEM)</label>
...
<div class="card" style="margin-bottom:24px;max-width:520px;">
  <h3>4. Request compliance CSID</h3>
  ...
  <form method="post" action="/admin/companies/<?= $company['id'] ?>/zatca/compliance-csid" style="display:flex;gap:8px;align-items:end;">
    ...
    <button type="submit" class="btn btn-primary" <?= empty($company['zatca_csr']) ? 'disabled' : '' ?>>Request compliance CSID</button>
  </form>
  <?php if (!empty($company['zatca_compliance_csid'])): ?>
    <p class="help-text" style="margin-top:10px;color:var(--success);">Compliance CSID is issued for this company.</p>
  <?php endif; ?>
</div>

<div class="card" style="margin-bottom:24px;max-width:520px;">
  <h3>5. Request production CSID</h3>
  ...
  <form method="post" action="/admin/companies/<?= $company['id'] ?>/zatca/production-csid" style="display:flex;gap:8px;align-items:end;">
    ...
    <button type="submit" class="btn btn-primary" <?= empty($company['zatca_compliance_csid']) ? 'disabled' : '' ?>>Request production CSID</button>
  </form>
  <?php if ($status === 'active'): ?>
    <p class="help-text" style="margin-top:10px;color:var(--success);">This company is live on ZATCA Phase 2 — invoices are reported automatically.</p>
  <?php endif; ?>
</div>
