...
<div class="kpi-grid" style="margin-bottom:24px;">
  <div class="kpi"><div class="label"><?= t('common.amount') ?></div><div class="value" style="font-size:16px;"><?= View::money((float)$payment['amount']) ?></div></div>
  <div class="kpi"><div class="label"><?= t('common.method') ?></div><div class="value" style="font-size:16px;"><?= View::e(strtoupper($payment['method'])) ?></div></div>
  <div class="kpi"><div class="label"><?= t('common.status') ?></div><div class="value" style="font-size:16px;"><span class="badge badge-<?= $payment['status']==='paid'?'green':($payment['status']==='pending'?'yellow':($payment['status']==='refunded'?'blue':'red')) ?>"><?= View::e($payment['status']) ?></span></div></div>
  <div class="kpi"><div class="label"><?= t('admin.payment.reviewed') ?></div><div class="value" style="font-size:16px;"><?= View::e($payment['reviewed_at'] ?: '—') ?></div></div>
</div>
...
<?php if (!empty($payment['proof_file_path'])): ?>
  <div class="card" style="margin-bottom:24px;max-width:520px;">
    ...
    <a href="<?= View::e($payment['proof_file_path']) ?>" target="_blank" rel="noopener" class="btn btn-outline"><?= t('admin.payment.view_uploaded_proof') ?></a>
  </div>
<?php endif; ?>
...
<div class="card" style="margin-bottom:24px;max-width:520px;">
  ...
  <form method="post" action="/admin/payments/<?= $payment['id'] ?>/update">
    <?= Csrf::field() ?>
    <div class="form-row">
      <div class="form-group"><label><?= t('common.amount') ?> (SAR)</label><input type="number" step="0.01" name="amount" value="<?= View::e((string)$payment['amount']) ?>"></div>
    ...
    <div class="form-row">
      <div class="form-group"><label><?= t('common.method') ?></label><input type="text" name="method" value="<?= View::e($payment['method']) ?>"></div>
      <div class="form-group"><label><?= t('common.reference') ?></label><input type="text" name="reference" value="<?= View::e($payment['reference']) ?>"></div>
    </div>
