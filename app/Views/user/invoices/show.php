...
<div class="page-head">
  <div>
    ...
    <p class="help-text" style="margin-top:4px;"><?= t('common.client') ?>: <?= View::e($client ? View::local($client, 'name') : '—') ?><?php if ($project): ?> · <?= t('common.project') ?>: <a href="/app/projects/<?= $project['id'] ?>"><?= View::e(View::local($project, 'name')) ?></a><?php endif; ?></p>
  </div>
...
<form method="get" action="/app/invoices/<?= $invoice['id'] ?>/pdf" target="_blank" style="display:flex;gap:8px;align-items:end;margin-bottom:20px;max-width:820px;">
  ...
  <?php if ($whatsappLink): ?>
    <a href="<?= View::e($whatsappLink) ?>" target="_blank" rel="noopener" class="btn btn-light" style="background:#25D366;color:#fff;border-color:#25D366;">💬 <?= t('common.send_whatsapp') ?></a>
  <?php endif; ?>
...
<div class="card" style="max-width:820px;">
  ...
  <?php if (!empty($invoice['vat_amount'])): $subtotal = (float)$invoice['total'] - (float)$invoice['vat_amount']; ?>
    <div style="text-align:right;font-size:14px;color:var(--muted);margin-top:14px;">
      <?= t('common.subtotal') ?>: <?= View::money($subtotal) ?><br>
      <?= t('common.vat') ?> (<?= View::e((string)$invoice['vat_rate']) ?>%): <?= View::money((float)$invoice['vat_amount']) ?>
    </div>
  <?php endif; ?>
  <div class="total-row" style="margin-top:6px;"><?= t('common.total') ?>: <?= View::money((float)$invoice['total']) ?></div>
  <?php if ($invoice['due_date']): ?><p class="help-text"><?= t('common.due') ?>: <?= View::e($invoice['due_date']) ?></p><?php endif; ?>
</div>
...
<?php if ((float) $invoice['retention_amount'] > 0): ?>
  <div class="card" style="max-width:820px;margin-top:20px;">
    ...
    <p class="help-text">
      <?= View::e((string)$invoice['retention_percent']) ?>% <?= t('user.invoices.withheld') ?>:
      <strong><?= View::money((float)$invoice['retention_amount']) ?></strong> ·
      <?= t('user.invoices.net_payable') ?>: <strong><?= View::money((float)$invoice['total'] - (float)$invoice['retention_amount']) ?></strong>
    </p>
    <?php if ($invoice['retention_released']): ?>
      <p class="help-text" style="color:var(--success);">✅ <?= t('user.invoices.released_on') ?> <?= View::e($invoice['retention_released_at']) ?></p>
    <?php else: ?>
...
?>
  <div class="card" style="max-width:820px;margin-top:20px;">
    ...
    <p class="help-text" style="margin-bottom:10px;">
      <?= t('common.status') ?>: <span class="badge badge-<?= $zColor ?>"><?= View::e($zLabel) ?></span>
...
<div class="card" style="max-width:820px;margin-top:20px;">
  ...
  <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-top:8px;">
    ...
    <button type="button" class="btn btn-sm btn-outline" onclick="navigator.clipboard.writeText('<?= View::e($shareUrl) ?>'); this.textContent='<?= t('common.copied') ?>';"><?= t('common.copy_link') ?></button>
    <a href="<?= View::e($shareUrl) ?>" target="_blank" class="btn btn-sm btn-outline"><?= t('common.preview') ?></a>
  </div>
</div>
