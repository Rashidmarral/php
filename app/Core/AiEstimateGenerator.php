...
class AiEstimateGenerator
{
    ...
    {
        return Settings::get('ai_enabled') === '1' && Settings::get('ai_api_key', '') !== '';
    }
...
PROMPT;
        ...
        if ($curlError || $httpCode !== 200 || !$response) {
            Settings::set('ai_last_error', $curlError ?: "HTTP {$httpCode}: " . substr((string) $response, 0, 300));
            return null;
        }
        ...
        if (!$text) {
            Settings::set('ai_last_error', 'Unexpected API response shape.');
            return null;
        }
        ...
        if (!is_array($parsed) || empty($parsed['items'])) {
            Settings::set('ai_last_error', 'Could not parse AI response as JSON.');
            return null;
        }
    ...
    private static function generateViaTemplateFallback(string $description): array
    {
        ...
        $templateItems = EstimateTemplateItem::forTemplate($bestTemplate['id']);
        $items = array_map(fn($ti) => [
