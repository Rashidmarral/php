 ...
 *    account, no API key, no setup; works the instant a phone number is on file. This is what
 *    "Send via WhatsApp" buttons use throughout the app.
 *
 * 2. sendMessage() — a real call to Meta's WhatsApp Business Cloud API for actually automated,
 *    no-click notifications (e.g. "your invoice was created"). This only works once a company has
 *    a real Meta WhatsApp Business Platform account (phone number ID + access token) — there's no
 *    way to fabricate that; outside an open 24-hour customer-service window, Meta also requires
 *    the message to use a pre-approved template rather than free-form text, which this sends as.
 */
class WhatsApp
{
    public static function isConfigured(): bool
    {
        return Settings::get('whatsapp_enabled') === '1'
